...
extension ValueBounds {
    /// 最大值与最小值的距离。
    var distance: Double { max - min }
    
    /// 合并另一个 `ValueBounds`，更新最大值和最小值。
    ///
    /// - Parameter other: 需要合并的另一个 `ValueBounds`。
    mutating func merge(other bounds: ValueBounds) {
        max = Swift.max(max, bounds.max)
        min = Swift.min(min, bounds.min)
    }
    
    func merging(other bounds: ValueBounds) -> ValueBounds {
        let max = Swift.max(max, bounds.max)
        let min = Swift.min(min, bounds.min)
        return ValueBounds(min: min, max: max)
    }
}
