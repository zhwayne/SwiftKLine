        ...
        ChartDescriptor {
            ...
            RendererGroup(height: candleHeight, padding: (12, 12)) {
                ...
                for type in mainIndicatorTypes {
                    if type == .ma {
                        MARenderer(configuration: .init(period: 5, color: .systemOrange))
                        MARenderer(configuration: .init(period: 10, color: .systemPink))
                        MARenderer(configuration: .init(period: 20, color: .systemTeal))
                    } else if type == .ema {
                        EMARenderer(configuration: .init(period: 5, color: .systemOrange))
                        EMARenderer(configuration: .init(period: 10, color: .systemPink))
                        EMARenderer(configuration: .init(period: 20, color: .systemTeal))
                    }
                }
                // Y轴
                YAxisRenderer()
                // 价格相关
                PriceIndicatorRenderer(
                    configuration: .init(
            ...
            for type in subIndicatorTypes {
                RendererGroup(height: indicatorHeight) {
                    ...
                    if type == .vol {
                        ...
                        RSIRenderer(configuration: .init(period: 6, color: .systemOrange))
                        RSIRenderer(configuration: .init(period: 12, color: .systemPink))
                        RSIRenderer(configuration: .init(period: 24, color: .systemTeal))
                    }
                }
            }
        }
...
extension KLineView {
    ...
    private func drawVisibleContent() {
        ...
        let visibleRect = layout.frameOfVisibleRange
        ...
        │  XXXXXX                                   │
        │               xxxxxxx                  xx │
        │xx       xxxxxxx     x    xxxxxx        xxx│
        │ xxxxxxxxx          ┼xxxxxx    xxxxxxxxxx  │
        └───────────────────────────────────────────┘
         */
        for (idx, group) in descriptor.groups.enumerated() {
            // 计算可见区域内每个 group 所在的位置
            var groupFrame = descriptor.frameOfGroup(at: idx, visibleRect: visibleRect)
            context.groupFrame = groupFrame
//            // 获取每个group的图例
//            let (legend, legendFrame) = group.legend(at: idx, context: context)
//
            groupFrame.origin.y += group.padding.top
            groupFrame.size.height -= (group.padding.top + group.padding.bottom)
            context.viewPort = groupFrame
            
            var extralValues: [[Any?]?] = Array(repeating: nil, count: group.renderers.count)
            var dataBounds: MetricBounds = .empty
            // 预设 extralValues
            // 计算可见区域内数据范围
            for (idx, renderer) in group.renderers.enumerated() {
                if let identifier = renderer.calculator?.identifier,
